...
class VRIC(object):
    ...
    def __init__(self, root='data', **kwargs):
        self.dataset_dir = root 
        self.train_dir = osp.join(self.dataset_dir, 'VRIC/train_images')
        self.query_dir = osp.join(self.dataset_dir, 'VRIC/probe_images')
        self.gallery_dir = osp.join(self.dataset_dir, 'VRIC/gallery_images')
        self.list_train_path = osp.join(self.dataset_dir, 'VRIC/vric_train.txt')
        self.list_query_path = osp.join(self.dataset_dir, 'VRIC/vric_probe.txt')
        self.list_gallery_path = osp.join(self.dataset_dir, 'VRIC/vric_gallery.txt')

        self._check_before_run()

        train, num_train_pids, num_train_imgs = self._process_dir(self.train_dir, self.list_train_path, relabel=True)
...
class VERI(object):
    ...
    def __init__(self, root='data', **kwargs):
        self.dataset_dir = root 
        self.train_dir = osp.join(self.dataset_dir, 'VeRi/image_train')
        self.query_dir = osp.join(self.dataset_dir, 'VeRi/image_query')
        self.gallery_dir = osp.join(self.dataset_dir, 'VeRi/image_test')
        self.list_train_path = osp.join(self.dataset_dir, 'VeRi/name_train.txt')
        self.list_query_path = osp.join(self.dataset_dir, 'VeRi/name_query.txt')
        self.list_gallery_path = osp.join(self.dataset_dir, 'VeRi/name_test.txt')
    ...
    def _process_dir(self, dir_path, list_path, relabel=False):
        ...
        for img_idx, img_info in enumerate(lines):
            ...
            pid, camid, _, _ = img_info.split('_')
            pid = int(pid)
            pid_container.add(pid)
        pid2label = {pid:label for label, pid in enumerate(pid_container)}
        ...
        for img_idx, img_info in enumerate(lines):
            ...
            pid, camid, _, _ = img_info.split('_')
            pid = int(pid)
            camid = int(camid.split('c')[1])
            if relabel: pid = pid2label[pid]
            img_path = osp.join(dir_path, img_info)
            dataset.append((img_path, pid, camid))
        num_imgs = len(dataset)
        num_pids = len(pid_container)

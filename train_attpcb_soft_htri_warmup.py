    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        if args.stepsize > 0:
            if args.warmup:
                ...
                scheduler.step()
        train(epoch, model, criterion_xent, criterion_htri, optimizer, trainloader, use_gpu=use_gpu, summary=writer, length=length)
    ...
    for batch_idx, (imgs, pids, _) in enumerate(trainloader):
        ...
        if args.htri_only:
            ...
            if isinstance(outputs, tuple):
                ...
                xent_loss = 0
                for per_partf in outputs:

                    xent_loss += criterion_xent(per_partf, pids)
        ...
        if summary is not None:
            if args.loss == 'xent,htri':
                ...
                summary.add_scalars('loss', {'Total loss': loss.item(), 'xentloss': xent_loss.item()})
...
def extract_feature(inputs,model):
    ...
    ff = torch.FloatTensor(inputs.size(0), 2048, 6).zero_()
    ff = ff.cuda()
    for i in range(2):
        if i==1:

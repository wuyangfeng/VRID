...
import models
from losses import CrossEntropyLabelSmooth, TripletLoss, DeepSupervision, SphereLoss
...
def main():
    ...
    model = models.init_model(name=args.arch, num_classes=dataset.num_train_pids, loss={'sphereloss'})
    print("Model size: {:.5f}M".format(sum(p.numel() for p in model.parameters())/1000000.0))

    criterion_sphere = SphereLoss()
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        if args.stepsize > 0:
            if args.warmup:
                ...
                scheduler.step()
        train(epoch, model, criterion_sphere, optimizer, trainloader, use_gpu)
...
def train(epoch, model, criterion_sphere, optimizer, trainloader, use_gpu):
    losses = AverageMeter()
    ...
    for batch_idx, (imgs, pids, _) in enumerate(trainloader):
        ...
        outputs = model(imgs)
        sphere_loss = criterion_sphere(outputs, pids)
        loss = sphere_loss
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        if (batch_idx+1) % args.print_freq == 0:
            print('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'Loss {loss.val:.4f} ({loss.avg:.4f})\t'.format(
                   epoch+1, batch_idx+1, len(trainloader), optimizer.param_groups[0]['lr'], batch_time=batch_time,

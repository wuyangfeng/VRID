...
def visualize_ranked_results(distmat, dataset, save_dir='log/ranked_results', topk=20):
    ...
    for q_idx in range(num_q):
        ...
        q_img = mpimg.imread(qimg_path)
        plt.figure()
        plt.subplot(3,10,1)
        plt.title('query_' + osp.basename(qimg_path).split('_')[0])
        plt.imshow(q_img)
        plt.axis('off')
        ...
        for g_idx in indices[q_idx,:]:
            ...
            if not invalid:
                ...
                g_img = mpimg.imread(gimg_path)
                plt.subplot(3,10,rank_idx+10)
                plt.title(osp.basename(gimg_path).split('_')[0])
                plt.imshow(g_img)
                plt.axis('off')
                rank_idx += 1
                if rank_idx > topk:
                    break
        plt.savefig(qdir+'/ranked.jpg')
        plt.close('all')
